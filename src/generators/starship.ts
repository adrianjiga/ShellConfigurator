...
function buildFormatString(modules: string[]): string {
  // character is always last on left, handle separately
  const parts = modules.map((m) => `$${m}`).join('');
  return parts;
}
...
export function generateToml(state: WizardState): string {
  ...
  const blocks = allModules
    .map((id) => moduleBlock(id, state))
    .join('\n\n');
  ...
  if (rightFormat) {
    lines.push(`right_format = "${rightFormat}"`);
  }

  lines.push(`add_newline  = true`, ``, blocks);
